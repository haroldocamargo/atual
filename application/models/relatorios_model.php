...
class Relatorios_model extends CI_Model {
    ...
    public function estoqueRapid(){
        
        $dataInicial = date('Y-m-01');
        $dataFinal = date("Y-m-t");
        $query = "SELECT estoque.*, produtos.descricao FROM estoque LEFT JOIN produtos ON estoque.produtos_id =  produtos.idProdutos WHERE data BETWEEN ? and ? ORDER BY dataAtualizacao asc";

        return $this->db->query($query, array($dataInicial,$dataFinal))->result();
    }


    public function estoqueCustom($dataInicial, $dataFinal, $tipo = null){
        ...
        if($tipo == 'entrada'){
            $whereTipo = "AND estoque.tipo = 'entrada'";
        }
        if($tipo == 'saida'){
            $whereTipo = "AND estoque.tipo = 'saida'";
        }
        
        $query = "SELECT estoque.*, produtos.descricao FROM estoque LEFT JOIN produtos ON estoque.produtos_id =  produtos.idProdutos WHERE estoque.data BETWEEN ? and ? $whereTipo";
    ...
    public function comprasCustom($dataInicial = null,$dataFinal = null,$cliente = null,$responsavel = null){
        $whereData = "";
        $whereCliente = "";
        $whereResponsavel = "";
        $whereStatus = "";
        if($dataInicial != null){
            $whereData = "AND dataCompra BETWEEN ".$this->db->escape($dataInicial)." AND ".$this->db->escape($dataFinal);
        }
        if($cliente != null){
            $whereCliente = "AND clientes_id = ".$this->db->escape($cliente);
        }
        ...
        $query = "SELECT compras.*,clientes.nomeCliente,usuarios.nome FROM compras LEFT JOIN clientes ON compras.clientes_id = clientes.idClientes LEFT JOIN usuarios ON compras.usuarios_id = usuarios.idUsuarios WHERE idCompras != 0 $whereData $whereCliente $whereResponsavel";
        return $this->db->query($query)->result();
    }
