...
class Conecte_model extends CI_Model {
	...
	public function getLastVendas($cliente){
	...
	public function getVendas($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array',$cliente){
	...
	public function getLastCompras($cliente){
		...
		$this->db->select('compras.*,usuarios.nome');
	...
	public function getCompras($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array',$cliente){
        ...
        $this->db->select($fields);
        $this->db->from($table);
        $this->db->join('usuarios', 'compras.usuarios_id = usuarios.idUsuarios', 'left');
        $this->db->where('clientes_id', $cliente);
        $this->db->limit($perpage,$start);
        if($where){
            $this->db->where($where);
