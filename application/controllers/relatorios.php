...
class Relatorios extends CI_Controller{
    ...
    public function estoque(){
        ...
        if(!$this->permission->checkPermission($this->session->userdata('permissao'),'rEstoque')){
           $this->session->set_flashdata('error','Você não tem permissão para gerar relatórios de estoque.');
           redirect(base_url());
        }

        $this->data['view'] = 'relatorios/rel_estoque';
        $this->load->view('tema/topo',$this->data);
    ...
    public function estoqueRapid(){
        ...
        $data['estoque'] = $this->Relatorios_model->estoqueRapid();

        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirEstoque', $data, true);
        pdf_create($html, 'relatorio_estoque' . date('d/m/y'), TRUE);
    }

    public function estoqueCustom(){
        ...
        $data['estoque'] = $this->Relatorios_model->estoqueCustom($dataInicial,$dataFinal,$tipo);
        $this->load->helper('mpdf');
        $html = $this->load->view('relatorios/imprimir/imprimirEstoque', $data, true);
        pdf_create($html, 'relatorio_estoque' . date('d/m/y'), TRUE);
    }
    ...
    public function comprasRapid(){
        ...
        $data['compras'] = $this->Relatorios_model->comprasRapid();
    ...
    public function comprasCustom(){
        ...
        $data['compras'] = $this->Relatorios_model->comprasCustom($dataInicial,$dataFinal,$cliente,$responsavel);
        $this->load->helper('mpdf');
        //$this->load->view('relatorios/imprimir/imprimirOs', $data);
        $html = $this->load->view('relatorios/imprimir/imprimirCompras', $data, true);
        pdf_create($html, 'relatorio_compras' . date('d/m/y'), TRUE);
